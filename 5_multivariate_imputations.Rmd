...
bf_fsh_bc <- bf(logFSH ~ s(bc_day) + (1 | id) + (1|id:cycle))

mod_mv_bc <- brm(
  bf_e_bc + bf_fe_bc + bf_p_bc + bf_t_bc + bf_ft_bc + bf_lh_bc + bf_fsh_bc + set_rescor(TRUE),
  data = biocycle,
...
bc_days$mv_est_fsh_bc <- round(preds_bc[, "Estimate", "logFSH"], 2)
...
bf_fsh_fc <- bf(logFSH ~ s(fc_day) + (1 | id) + (1|id:cycle))

mod_mv_fc <- brm(
  bf_e_fc + bf_fe_fc + bf_p_fc + bf_t_fc + bf_ft_fc + bf_lh_fc + bf_fsh_fc + set_rescor(TRUE),
  data = biocycle,
  control = list(adapt_delta = 0.95, max_treedepth = 20),
...
fc_days$mv_est_fsh_fc <- round(preds_fc[, "Estimate", "logFSH"], 2)
...
bf_fsh_lh <- bf(logFSH ~ s(lh_day) + (1 | id) + (1|id:cycle))

# PBFW not modeled in LH model

mod_mv_lh <- brm(
  bf_e_lh + bf_fe_lh + bf_p_lh + bf_t_lh + bf_ft_lh + bf_lh_lh + bf_fsh_lh + set_rescor(TRUE),
  data = biocycle_lh,
  control = list(adapt_delta = 0.95, max_treedepth = 20),
...
lh_days$mv_est_fsh_lh <- round(preds_lh[, "Estimate", "logFSH"], 2)
