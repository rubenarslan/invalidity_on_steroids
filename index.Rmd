...
summary_table <- function(Hormone) {
  ...
  # print(table(comp$Sample))
  
  comp <- comp %>% 
    pivot_wider(name, Sample) %>% 
    rename(rowname = name)

  colnames(comp) <- str_trim(str_replace(str_replace(
    colnames(comp), Hormone, ""), "_", " "))
    ...
    mutate(rowgroup = case_when(
      ...
      rowname %in%  c("icc", "Variance Ratio", "var_id", "var_id_cycle", "rmse_icc") ~ "Interindividual differences",
