...
biocycle %>% group_by(id, cycle) %>%
  filter(!noserum) %>% 
  filter(n() >= 7, !is.na(cycle_length_bl)) %>% 
...
xtabs(~ is.na(serum_LH_surge_day), biocycle)
xtabs(~ is.na(peak_day), biocycle)
xtabs(~ is.na(serum_LH_surge_day) + is.na(peak_day), biocycle)
xtabs(~ is.na(serum_LH_surge_day) + is.na(serum_LH_peak_day), biocycle)
xtabs(~ is.na(serum_LH_surge_day) + is.na(peak_day), biocycle)
...
## Urine validity
```{r}
table(is.na(biocycle$peak_day))
table((biocycle$peak_day))
options(pillar.sigfig = 2)
biocycle %>% filter(between(peak_day - cday, -5, 5)) %>% 
  group_by(peak_day - cday) %>% 
  summarise(
  cor(ESTRGEN, E3g_monitor_peak),
  cor(ESTRGEN, E3g_monitor_peak, method = "s"),
  cor(log(ESTRGEN), log(E3g_monitor_peak)),
  n_E=  cor.test(ESTRGEN, E3g_monitor_peak)$parameter + 2,
  cor(LH, LH_monitor_peak),
  cor(LH, LH_monitor_peak, method = "s"),
  cor(log(LH), log(LH_monitor_peak)),
  n_LH =  cor.test(LH, LH_monitor_peak)$parameter + 2
)
```


## Anovulatory cycles vs. rest
